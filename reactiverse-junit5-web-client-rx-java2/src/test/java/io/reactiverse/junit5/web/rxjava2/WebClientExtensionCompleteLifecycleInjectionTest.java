 ...
 */
package io.reactiverse.junit5.web.rxjava2;
...
class WebClientExtensionCompleteLifecycleInjectionTest {
  ...
  @BeforeEach
  void rightBefore(Vertx vertx, VertxTestContext testContext, WebClient client) {
    assertThat(client).isNotNull();
    assertThat(testContext).isNotNull();
    testContext.completeNow();
  }
  ...
  @AfterEach
  void rightAfter(Vertx vertx, VertxTestContext testContext, WebClient client) {
    assertThat(client).isNotNull();
    assertThat(testContext).isNotNull();
    testContext.completeNow();
  }
  ...
  @AfterAll
  static void inTheEnd(Vertx vertx, VertxTestContext testContext, WebClient client) {
    assertThat(client).isNotNull();
    assertThat(testContext).isNotNull();
    testContext.completeNow();
  }
